...
function buildSessionStats(table: GenericTable, seatId: string): {
  ...
  const avgTableChips = humanChips.length > 0
    ? Math.round(humanChips.reduce((a, b) => a + b, 0) / humanChips.length) : 10000;
...
function resetToAnte(table: GenericTable): void {
  ...
  let nextPlayers: Player[] = basePlayers.map(p => {
    ...
    const newChips = isBotBusted ? 10000 : Math.max(0, p.chips);
    return {
      ...p,
      cards: [],
      bet: 0,
...
async function broadcastChatFiltered(table: GenericTable, senderSeat: string): Promise<void> {
  ...
  for (const [recipientSeat, ws] of Array.from(table.connections.entries())) {
    ...
    try {
      let personalState = maskStateForPlayer(baseState, recipientSeat, pub);
      if (table.state.phase === 'SHOWDOWN') {
      ...
      ws.send(JSON.stringify({
        type: 'mode:snapshot',
        state: personalState,
        sessionStats: buildSessionStats(table, recipientSeat),
      }));
    } catch {}
  }
