import { Player, CardType, GameState } from '../gameTypes';
...
export const getNextActivePlayerIndex = (players: Player[], currentIndex: number, skipAllIn = true): number => {
...
export const moveDealer = (players: Player[]): Player[] => {
  ...
  return players.map((p, i) => ({ ...p, isDealer: i === nextDealerIdx }));
};
...
export const isRoundOver = (players: Player[], currentBet: number): boolean => {
  ...
  return activePlayers.every(p => p.hasActed) && activePlayers.every(p => p.bet === currentBet);
};
