import { CardType, HandEvaluation } from './gameTypes';
...
export function evaluateBestHand(
  ...
  let bestHighValue = -1, bestHighStr = '', bestHighIndices = { hole: [] as number[], comm: [] as number[] };
  let bestLowValue = 9999999, bestLowStr = '', bestLowIndices = { hole: [] as number[], comm: [] as number[] };
  ...
  return {
    ...
    highEval: { description: bestHighStr, usedHoleCardIndices: bestHighIndices.hole, usedCommunityCardIndices: bestHighIndices.comm },
    lowEval: { description: bestLowValue < 9999999 ? bestLowStr : 'No Low', usedHoleCardIndices: bestLowIndices.hole, usedCommunityCardIndices: bestLowIndices.comm }
  };
}
...
function eval5Cards(cards: CardType[]) {
  ...
  let highValue = 0, highName = '';
  if (isStraight && isFlush) { highValue = 8000000 + ranks[0]; highName = 'Straight Flush'; }
  else if (counts[0] === 4) { highValue = 7000000; highName = 'Four of a Kind'; }
  else if (counts[0] === 3 && counts[1] === 2) { highValue = 6000000; highName = 'Full House'; }
  else if (isFlush) { highValue = 5000000; highName = 'Flush'; }
  else if (isStraight) { highValue = 4000000; highName = 'Straight'; }
  else if (counts[0] === 3) { highValue = 3000000; highName = 'Three of a Kind'; }
  else if (counts[0] === 2 && counts[1] === 2) { highValue = 2000000; highName = 'Two Pair'; }
  else if (counts[0] === 2) { const pr = parseInt(Object.keys(rankCounts).find(k => rankCounts[parseInt(k)] === 2) || '0'); highValue = 1000000 + pr * 1000; highName = 'Pair'; }
  else { highValue = ranks[0]; highName = 'High Card'; }

  let lowValue = 9999999, lowName = 'No Low';
  const lowRanks = ranks.map(r => r === 14 ? 1 : r);
  const uniqueLowRanks = Array.from(new Set(lowRanks)).sort((a, b) => b - a);
