...
interface KamikazeActionBarProps {
  phase: string;
  isDrawPhase: boolean;
...
function ChipIcon() {
  return (
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none" style={{ display: 'inline', verticalAlign: 'middle', marginRight: 3 }}>
      <circle cx="6" cy="6" r="5.5" fill="#1e1e1e" stroke="#facc15" strokeWidth="0.75"/>
      <circle cx="6" cy="6" r="3.5" fill="none" stroke="#ef4444" strokeWidth="0.75"/>
    </svg>
  );
}
...
function TutorialPanel() {
  return (
    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.25 }} style={{ overflow: 'hidden' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, padding: '10px 4px 6px', borderTop: `1px solid ${R(0.2)}` }}>
        {[
          { icon: '🎴', label: '3+2+1 SUITS', sub: 'Three different suits' },
          { icon: '🔢', label: 'NO PAIRS', sub: 'All 6 ranks unique' },
          { icon: '⚡', label: 'HI OR LO', sub: 'Declare after draws' },
        ].map(step => (
          <div key={step.label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            <span style={{ fontSize: 18 }}>{step.icon}</span>
            <span style={{ fontSize: 11, fontFamily: 'monospace', color: '#ef4444', fontWeight: 700, letterSpacing: '0.06em', textAlign: 'center', lineHeight: 1.2 }}>{step.label}</span>
  ...
  return (
    <div style={{ width: '100%' }}>
      <div style={{ padding: '8px 12px 0' }}>
        ...
        {isDrawPhase && (
          <div style={{ display: 'flex', gap: 10 }}>
            <button style={stayBtn} disabled={!canAct} onClick={canAct ? onStay : undefined} data-testid="button-stay">STAY</button>
        ...
        {isDeclare && (
          myHasActed ? (
            <div style={{ textAlign: 'center', padding: '14px 0 10px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
              <div style={{ fontSize: 11, fontFamily: 'monospace', color: Y(0.7), letterSpacing: '0.14em' }}>YOU DECLARED</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ fontSize: 11, fontFamily: 'monospace', color: Y(0.8), letterSpacing: '0.12em', textAlign: 'center', paddingTop: 4 }}>DECLARE HIGH OR LOW</div>
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => onAction('declare', { declaration: 'HIGH' })} data-testid="button-declare-high"
                  style={{ flex: 1, padding: '16px 8px', borderRadius: 12, fontSize: 16, fontFamily: 'monospace', fontWeight: 900, letterSpacing: '0.14em', textTransform: 'uppercase', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #92400e, #facc15)', color: '#000', boxShadow: `0 0 22px ${Y(0.6)}, 0 4px 14px rgba(0,0,0,0.4)`, WebkitTapHighlightColor: 'transparent' }}>
                  HIGH
                </button>
                <button onClick={() => onAction('declare', { declaration: 'LOW' })} data-testid="button-declare-low"
                  style={{ flex: 1, padding: '16px 8px', borderRadius: 12, fontSize: 16, fontFamily: 'monospace', fontWeight: 900, letterSpacing: '0.14em', textTransform: 'uppercase', cursor: 'pointer', background: 'linear-gradient(135deg, #1e3a5f, #3b82f6)', color: '#fff', boxShadow: `0 0 22px ${B(0.55)}, 0 4px 14px rgba(0,0,0,0.4)`, WebkitTapHighlightColor: 'transparent' }}>
                  LOW
                </button>
              </div>
      ...
      <button onClick={() => setTutorialOpen(o => !o)} data-testid="button-tutorial-toggle"
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, width: '100%', padding: '6px 12px', background: 'none', border: 'none', cursor: 'pointer', color: R(0.7), fontSize: 11, fontFamily: 'monospace', letterSpacing: '0.1em', textTransform: 'uppercase', WebkitTapHighlightColor: 'transparent' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 14px 10px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <button onClick={onRebuy} data-testid="button-rebuy"
          style={{ width: 28, height: 28, borderRadius: '50%', background: 'linear-gradient(135deg, #b91c1c, #ef4444)', border: 'none', cursor: 'pointer', color: '#fff', fontSize: 16, fontWeight: 700, lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 10px ${R(0.4)}`, WebkitTapHighlightColor: 'transparent' }}>
          +
        </button>
      </div>
    </div>
