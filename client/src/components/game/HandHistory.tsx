...
const RESULT_STYLES: Record<string, { label: string; bg: string; text: string }> = {
  ...
  push: { label: "PUSH", bg: "bg-white/10", text: "text-white/60" },
  rollover: { label: "ROLLOVER", bg: "bg-amber-500/15", text: "text-amber-400" },
  folded: { label: "FOLDED", bg: "bg-white/10", text: "text-white/60" },
};
          ...
          <div className="p-5 sm:p-6 pt-10">
            ...
            {history.length > 0 && (
              <div className="flex items-center gap-3 mb-5 p-3 rounded-lg bg-white/[0.03] border border-white/[0.06]">
                <div className="flex-1">
                  <div className="text-[12px] text-white/60 font-mono uppercase tracking-wider">Net Result</div>
                  <div className={`text-lg font-bold font-mono ${netChips > 0 ? "text-emerald-400" : netChips < 0 ? "text-red-400" : "text-white/60"}`}>
                    {netChips > 0 ? "+" : ""}{netChips === 0 ? "Even" : `$${netChips}`}
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-[12px] text-white/60 font-mono uppercase tracking-wider">W / L</div>
            ...
            {history.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-white/60 text-sm">No hands played yet</p>
                <p className="text-white/60 text-xs mt-1">Results will appear here after each hand</p>
              </div>
            ) : (
...
function HandRow({ hand, showMode }: { hand: HandRecord; showMode: boolean }) {
  ...
  const modeColor = MODE_COLORS[hand.mode] || "text-white/60";
    ...
    <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] transition-colors" data-testid={`card-hand-${hand.id}`}>
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2">
          <span className={`text-[12px] font-mono font-bold px-1.5 py-0.5 rounded ${style.bg} ${style.text}`}>
            {style.label}
          </span>
          {showMode && (
            <span className={`text-[12px] font-mono ${modeColor}`}>
              {hand.modeName}
            </span>
          )}
        </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right shrink-0">
          <div className={`text-sm font-mono font-bold ${hand.chipChange > 0 ? "text-emerald-400" : hand.chipChange < 0 ? "text-red-400" : "text-white/60"}`}>
